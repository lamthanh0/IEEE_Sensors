function OP_SPRS_EXACT=SPRS_INID_EXACT(PdB,xR,ORR,Eta,AP,Cth,PL)
OP_SPRS_EXACT   = zeros(1,length(PdB));
for aa = 1 : length(PdB)
    OP_SPRS_EXACT(aa) = ham(PdB(aa),xR,ORR,Eta,AP,Cth,PL);
end
%
OP_SPRS_EXACT;
%semilogy(PdB,OP_PRS,'g-'); grid on;hold on;
end
%
...
for aa = 1 : MM
    ...
    for bb = 1 : MM - 1
        ...
        for cc = 1 : row
            ...
            ff2    = @(xx) LRD(aa)./(LRD_sum+LRD(aa)).*ORR.*exp(-ORR.*xx).*...
                sqrt(4.*(LRD_sum+LRD(aa)).*LSR(aa).*(gth-kap.*gth.*xx)./kap./PP).*besselk(1,sqrt(4.*(LRD_sum+LRD(aa)).*LSR(aa).*(gth-kap.*gth.*xx)./kap./PP));
